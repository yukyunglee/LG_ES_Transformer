...
def get_dataloader(args):
    ...
    if normal:
        ...
        full.to_csv(f'{root_path}/normalized_{data_name[:-4]}_val_{valid_ratio}_tst_{test_ratio}.csv')
    ...
    full_dataloader = torch.utils.data.DataLoader(
        full_dataset, batch_size=batch_size, shuffle=args.shuffle, num_workers=4, drop_last=False
    )
    trn_dataloader = torch.utils.data.DataLoader(
        trn_dataset, batch_size=batch_size, shuffle=args.shuffle, num_workers=4, drop_last=False
    )
    dev_dataloader = torch.utils.data.DataLoader(
        dev_dataset, batch_size=batch_size, shuffle=args.shuffle, num_workers=4, drop_last=False
    )
    tst_dataloader = torch.utils.data.DataLoader(
        tst_dataset, batch_size=batch_size, shuffle=args.shuffle, num_workers=4, drop_last=False
    )

    return full_len, trn_len, dev_len, tst_len, full_dataloader, trn_dataloader, dev_dataloader, tst_dataloader
...
def make_matplt(df: pd.DataFrame, data_name, trn_len, tst_len, save_path=None):
    ...
    ax.axvline(x=df.timestamp[trn_len], label='train {}'.format(df.timestamp[trn_len]), c='k')
    ax.axvline(x=df.timestamp[len(df) - tst_len], label='test {}'.format(df.timestamp[len(df) - tst_len]), c='k')
